...
import com.google.ai.cxpAssistant.GenerativeViewModelFactory
...
internal fun ChatRoute(
    chatViewModel: ChatViewModel = viewModel(factory = GenerativeViewModelFactory), backHome:()-> Unit
) {
    ...
    Scaffold(
        containerColor = Color(0xffe8ebed),
        topBar = {TopAppBar(colors = TopAppBarDefaults.topAppBarColors(Color.White),
            title =
            { Text(text = "CXP Assistant", fontWeight = FontWeight.SemiBold) },
            actions = {IconButton(onClick = {backHome()}) {
                Icon(
